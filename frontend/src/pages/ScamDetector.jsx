...
const RULES = [
  { id: "urgent", label: "Urgency / pressure language", patterns: [/urgent/i, /immediately/i, /within\s+\d+\s*(hours?|mins?|minutes?)/i, /final\s+warning/i, /last\s+chance/i, /act\s+now/i], weight: 18 },
  { id: "payment", label: "Payment / transfer demand", patterns: [/pay\s+now/i, /transfer/i, /upi/i, /bank/i, /wallet/i, /crypto/i, /bitcoin/i, /gift\s*card/i, /fine/i, /penalty/i], weight: 22 },
  { id: "impersonation", label: "Authority impersonation", patterns: [/police/i, /court/i, /cyber\s*cell/i, /income\s*tax/i, /gst/i, /legal\s*notice/i, /advocate/i, /law\s*firm/i, /government/i, /ministry/i], weight: 16 },
  { id: "links", label: "Suspicious links", patterns: [/(bit\.ly|tinyurl|t\.co|rb\.gy|cutt\.ly)/i, /https?:\/\/[^\s]+/i, /\b[a-z0-9-]+\.(xyz|top|click|info|shop|site|live)\b/i], weight: 18 },
  { id: "personal", label: "Asking for personal data / OTP / passwords", patterns: [/\botp\b/i, /password/i, /\bpin\b/i, /\baadhaar\b/i, /\bpan\b/i, /\bcard\b/i, /\bcvv\b/i, /\baccount\s*number\b/i, /\blogin\b/i], weight: 24 },
  { id: "threats", label: "Threats / intimidation", patterns: [/arrest/i, /warrant/i, /case\s+filed/i, /legal\s+action/i, /jail/i, /freeze/i, /block/i, /blacklist/i], weight: 20 },
];
...
function scoreText(text) {
  ...
  for (const rule of RULES) {
    ...
    if (matched) { score += rule.weight; hits.push(rule.id); }
  }
      ...
      <nav className={NAV_BASE}>
        <div className={NAV_CONTAINER}>
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className={BACK_BUTTON} aria-label={ARIA_LABELS.GO_BACK}>
              ...
              <div>
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <p className={`${RISK_LABEL_BASE} ${
                      risk.tone === "high" ? "bg-rose-500/15 border border-rose-500/25 text-rose-800 dark:text-rose-200"
                        : risk.tone === "mid" ? "bg-amber-500/15 border border-amber-500/25 text-amber-800 dark:text-amber-200"
                  ...
                  <div className="w-28">
                    <div className="h-2.5 rounded-full bg-slate-100 dark:bg-white/10 overflow-hidden">
                      <div className="h-full rounded-full bg-gradient-to-r from-nyaya-500 to-blue-600" style={{ width: `${analysis.score}%` }} />
                    </div>
                ...
                <div className="mt-6">
                  ...
                  <div className="space-y-2">
                    {RULES.map((r) => {
                      ...
                      return (
                        <div key={r.id} className={`${RULE_ITEM_BASE} ${
                          hit ? "border-nyaya-500/30 bg-nyaya-500/10 text-nyaya-900 dark:text-nyaya-200"
                            : "border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-white/5"
                        }`}>
                          <div className="flex items-center gap-2">
                            {hit ? <BadgeAlert className="w-4 h-4 text-amber-600 dark:text-amber-300" /> : <BadgeCheck className="w-4 h-4 text-slate-400 dark:text-slate-500" />}
                            <span className={`text-sm ${hit ? "text-slate-800 dark:text-slate-100" : "text-slate-605 dark:text-slate-400"}`}>{r.label}</span>
                          </div>
                          <span className={`text-xs font-semibold ${hit ? "text-slate-700 dark:text-slate-200" : "text-slate-500"}`}>+{r.weight}</span>
                        </div>
                      );
                    })}
                  </div>
                  ...
                  <div className={WHATS_NEXT_CARD}>
                    ...
                    <ul className="mt-2 text-sm text-slate-650 dark:text-slate-400 space-y-1 list-disc list-inside">
                      {SCAM_TIPS.map((tip, i) => <li key={i}>{tip}</li>)}
                    </ul>
                  </div>
                </div>
              </div>
